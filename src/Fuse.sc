...
(class Fuse of Actor
	...
	(method (doit)
		(super doit: &rest)
		(if track (self x: (- gPEventX 12) y: (- gPEventY 8)))
	)
	...
	(method (drop)
		...
		(self stopUpd:)
		(if (not local2)
			(gSq5IconBar enable: 1 6)
			(if (gSq5IconBar curInvIcon?) (gSq5IconBar enable: 5))
		...
		(if socket
			(switch socket
				...
				(3
					(self y: (+ (self y?) 50) z: 50)
					(global2 drawPic: 51 100)
				)
				(5
					(if (proc0_1 95) (proc0_3 95))
					(socket5 occupied: 1)
				)
			)
			(Display
	...
	(method (pickup)
		...
		(if socket
			...
			(gSq5IconBar disable: 1 6)
			(if (gSq5IconBar curInvIcon?) (gSq5IconBar disable: 5))
			(= track 1)
		)
	)
)
		...
		(switch theVerb
			...
			(28
				(if
					...
					(if (and (== socketNum 5) (proc0_1 95)) (proc0_3 95))
					(extraFuse
						init:
						x: (self x?)
						y: (self y?)
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			...
			(1
				(if (fuseList size?)
					(if (socketList size?)
						(gTestMessager say: 0 0 0 1 self)
					else
						(= cycles 1)
					)
				else
			...
			(2
				...
				(if (proc999_5 (temp0 origHole?) 2 5)
					(if (and (socket2 occupied?) (socket5 occupied?))
						(gEgo get: 4)
						(fuseList delete: temp0)
						(temp0 dispose:)
						(gTestMessager say: 5 0 0 1 self)
					else
			...
			(4
				(if (not (socket2 occupied?))
					(if (not (proc0_1 72))
						(gTestMessager say: 4 0 1 1 self)
						(fuse5 dispose:)
						(socket5 occupied: 0)
					else
						(= cycles 1)
...
(instance schematic of Feature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1
				(cond 
					((proc999_4 151 103 171 112 gPEventX gPEventY) (= temp0 1))
					((proc999_4 173 103 191 112 gPEventX gPEventY) (= temp0 2))
					((proc999_4 192 103 210 112 gPEventX gPEventY) (= temp0 3))
					((proc999_4 151 118 171 126 gPEventX gPEventY) (= temp0 4))
					((proc999_4 173 118 191 126 gPEventX gPEventY) (= temp0 5))
					((proc999_4 192 118 210 126 gPEventX gPEventY) (= temp0 6))
				)
				(if temp0 (gTestMessager say: 1 0 0 temp0))
			)
