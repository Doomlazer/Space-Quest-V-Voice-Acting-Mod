...
(instance rm315 of Rm
	...
	(method (init)
		...
		(switch gGModNum
			(305
				(if (proc0_1 21)
					(global2 setScript: sWD40Explodes)
					(global2 style: 14)
				else
					(global2 setScript: sEnterViaTunnelE)
				)
			)
			(else 
				(global2 setScript: sEnterViaTunnelB)
			)
		)
		(if (not (proc0_1 21))
...
code_05a3:
	...
	(method (dispose)
		(gSq5Music2 stop:)
		(theMusic3 dispose:)
		(if (gOldWH contains: rock) (gOldWH delete: rock))
		(DisposeScript 991)
		(super dispose:)
	)
		...
		(switch (= state newState)
			(0
				...
				(if register
					...
					(if (== local0 5) (global2 setScript: sRogDiesInTunnel))
				)
			)
			(1
				(cond 
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(cond 
					((< local34 10) (gEgo cel: local34))
					((> local34 11) (gEgo cel: (- local34 2)))
					(else (gEgo cel: 9))
				)
				(if (== local34 12)
					(gSq5Music2 number: 5041 setLoop: 1 play:)
...
(instance sWD40Fires of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if
				(and (< (gEgo y?) 139) (not (global2 script?)))
					(wd40 hide:)
					(self dispose:)
				else
					(proc0_2 66)
...
(instance sWD40EntersCave of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(if
				(and (proc999_4 200 50 225 80 gEgo) (== local0 5))
					(global2 setScript: sRogDies)
					(self dispose:)
				)
				(= local0 0)
				(= cycles 1)
		...
		(switch theVerb
			(4
				(if
					...
					(global2 setScript: sClimbUptoRock)
				)
			)
			(3
				(if
			...
			(21
				(if (< (gEgo distanceTo: self) 26)
					(if (== local0 5)
						(global2 setScript: sBounceRock)
					else
						(gTestMessager say: 10 0 0 0)
					)
				else
