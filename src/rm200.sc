...
(procedure (localproc_021f)
	(cond 
		...
		(else (gSq5Music1 number: 101 loop: -1 play: 75))
	)
)

(procedure (localproc_0287)
	(cond 
		((or (proc0_1 84) (>= (eureka puke?) 4))
			(if (!= (gSq5Music2 number?) 105)
				(gSq5Music2 number: 105 loop: -1 play:)
			)
		)
		((!= (gSq5Music2 number?) 206) (gSq5Music2 number: 206 loop: -1 play:))
	)
)
...
(instance rm200 of Rm
	...
	(method (init)
		...
		(proc958_0 132 19)
		(gOldWH addToFront: chair northDoor)
		(global2
			addObstacle:
				((Polygon new:)
					type: 2
	...
	(method (dispose)
		(PalVary pvUNINIT)
		(gOldWH delete: chair northDoor)
		(if (proc0_1 0) (gTheDoits delete: sCyclePal))
		(super dispose:)
	)
)
		...
		(switch (= state newState)
			(0
				...
				(if (proc999_5 gGModNum 100 201)
					...
					(proc0_6 0 2)
					(gEgo init: posn: 161 122 setScale: Scaler 110 70 152 111)
				)
				(switch gGModNum
					(165
						(gEgo setPri: 3)
					...
					(201
						(cond 
							((proc0_1 85) (= next sAbandonShip))
							((and (proc0_1 108) (== gEurekaCurLocation 5))
								(if (proc0_1 86)
									(= next sStandUp)
									(proc0_3 108)
								else
							...
							(
							(and (== (eureka puke?) 4) (not (proc0_1 86))) (= next sAbandonShip))
							((== (eureka puke?) 2) (= next sBlobKills))
							((proc0_1 106) (= next sKUComment))
							(else (= next sStandUp))
						)
						(if (not (proc0_1 86))
							(flo init:)
...
(instance sKUComment of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(gTestMessager say: 9 0 0 1 self)
			)
			(4
				(gTestMessager say: 9 0 0 2 self)
			)
			(5
				(gSQ5 handsOn:)
				(self dispose:)
...
(instance sDrooleAbandonShip of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(cond 
					((and (< (eureka puke?) 4) (proc0_1 84)) (gTestMessager say: 4 0 1 1 self))
					(
					(and (== (eureka curLocation?) 4) (== global127 3)) (gTestMessager say: 4 0 3 1 self))
					(else (gTestMessager say: 4 0 1 2 self))
				)
			)
			(5 (= seconds 1))
			(6
...
(instance sExitNorth of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(cond 
					((or (proc0_1 86) (== global130 0)) (= global130 0))
					((== global170 1) (= global130 4))
					((== global126 1) (= global130 1))
					((proc0_1 61) (= global130
						(switch (Random 0 1)
							(0 1)
							(1 2)
						)))
				...
				(if (== (gSq5Music2 number?) 206) (gSq5Music2 fade:))
				(global2 newRoom: 225)
				(self dispose:)
			)
		)
...
(instance sFloGoofOff of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (not (gOldCast contains: floHead1))
					(floHead1 init:)
				)
			...
			(4
				(if (gOldCast contains: floHead1) (floHead1 dispose:))
...
(instance chair of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4
				(global2 setScript: sTakeCommand)
			)
			(3)
			(else 
				(super doVerb: theVerb &rest)
...
(instance northDoor of Prop
	...
	(method (init)
		...
		(self
			ignoreActors:
			approachVerbs: 4 3
			cel: (if (proc999_5 gGModNum 225) (self lastCel:) else 0)
		)
	)
