...
(instance pad of Prop
	...
	(method (handleEvent pEvent &tmp pEventMessage temp1 [temp2 20])
		(if (and (gUser controls?) (gUser input?))
			(cond 
				(
				(== (gSq5IconBar curIcon?) (gSq5IconBar at: 1)) (super handleEvent: pEvent &rest))
				(
					(and
						(not (& (pEvent type?) evVERB))
				...
				(
					...
					(cond 
						...
						(
						(and (== pEventMessage KEY_TAB) (not (proc241_3)))
							(pEvent claimed: 1)
