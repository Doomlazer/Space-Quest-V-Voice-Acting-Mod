...
(procedure (localproc_02b6 param1 &tmp temp0 temp1 temp2 temp3 temp4 [temp5 10] [temp15 10] [temp25 10] [temp35 10] [temp45 12])
	...
	(return
		(if
			...
			(cond 
				(
				(not (Message msgGET temp4 temp0 temp1 temp2 temp3)) (proc921_0 {Can't find message!}) (return 0))
				(
				(not (Message msgSIZE temp4 temp0 temp1 temp2 temp3)) (proc921_0 {Message contains no text!}) (return 0))
				(else
					(param1
						noun: temp0
						verb: temp1
...
(class DlgWindow of SysWindow
	...
	(method (editPosn &tmp newEvent newEventX temp2 temp3 temp4)
		(if local1
			...
			(= temp4 (- right left))
			(= newEventX
				(proc999_3 0 (proc999_2 newEventX (- 320 temp4)))
			)
	...
	(method (editFont &tmp [temp0 25] temp25)
		(= temp25
			(Print
				...
				addButton: gFont {__User__} 100 14
				addButton: global23 {__Small__} 100 28
				addButton: global26 {___Big___} 100 42
...
(class DialogEditor of List
	...
	(method (init)
		(= theGSq5Win gSq5Win)
		(StrCpy @local42 {DialogEditor__v1.1})
		((= gSq5Win SysWindow) color: 0 back: 255)
		(gSQ5 setCursor: 999)
		(self changeState: 0)
	)
		...
		(switch (pEvent type?)
			...
			(evKEYBOARD
				...
				(switch (pEvent message?)
					...
					(KEY_f
						(if (proc999_5 state 2 1 3) (curItem editFont:))
					)
					(KEY_h
						(switch state
							(0
	...
	(method (exit &tmp [temp0 100] temp100 newFile temp102)
		(if (not local1) (DlgWindow create:))
		(if (not local2) (Format @local2 {%d.dlg} gModNum))
		(if
			(not
				(= temp100
					(Print
		...
		(if
			...
			(proc921_1 {Error opening '%s'} (newFile name?))
			(newFile dispose:)
			(return 0)
		)
		(newFile
