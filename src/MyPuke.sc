...
(instance sFallOff of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(gTestMessager say: 14 0 0 1 self)
			)
			(5
...
(instance sDoComm of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(proc0_10 226 25)
				(gTestMessager say: 9 32 0 0 self)
			)
			(3
				(RogersHead setCel: 0)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(global2 drawPic: 103 (global2 style?))
				(both init:)
				(= seconds 1)
			)
			(3 (= seconds 2))
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(13
				(cond 
					(local0 (= cycles 1))
					((== sGiveFrock (global2 script?)) (= seconds 3))
					(else (global2 setScript: sByeBea))
				)
			)
			(14
...
(instance RogersBody of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(32
				(if (not local1) (global2 setScript: sDoComm))
			)
			(else 
